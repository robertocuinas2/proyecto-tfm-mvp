  ...
  return (
    <div className="grid gap-3 lg:grid-cols-3">
      {stats.map(({ label, value, sub, color, Icon }) => (
        <div
          key={label}
          className="rounded-lg border border-tv-border bg-tv-surface p-4"
        >
          ...
          <div className={`mt-2 font-heading text-4xl font-bold ${color}`}>
            {value}
          </div>
...
function ZoneCard({ zone, tasks }: { zone: Zone; tasks: Task[] }) {
  const programadas = tasks.filter(
    (task) => task.estado === "programada",
  ).length;
  ...
  const urgentes = tasks.filter(
    (task) => task.es_urgente && task.estado !== "ejecutada",
  ).length;
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {zone.descripcion && (
            <p className="mt-1 line-clamp-2 text-xs leading-5 text-tv-dim">
              {zone.descripcion}
            </p>
          )}
        </div>
        <span
          className={`shrink-0 rounded-full border px-2.5 py-1 text-[10px] font-extrabold uppercase ${statusStyles[status]}`}
        >
          {status}
        </span>
      </div>
      ...
      <div className="mt-4">
        <div className="mb-1 flex items-center justify-between text-xs font-semibold text-tv-dim">
          <span>
            {ejecutadas} / {total} tareas completadas
          </span>
      ...
      <div className="mt-4 grid grid-cols-4 gap-2">
        {[
          ...
          <div
            key={label}
            className="rounded-lg bg-tv-surface2 px-2 py-2 text-center"
          >
            <div className={`font-heading text-xl font-bold ${color}`}>
              {value}
            </div>
            <div className="mt-0.5 text-[10px] font-bold uppercase text-tv-dim">
              {label}
            </div>
          </div>
        ))}
...
export default function ZonesPage() {
  ...
  const totalDelayed = tasks.filter(
    (task) => task.estado === "retrasada",
  ).length;
      ...
      <div className="border-b border-tv-border px-6 py-5 lg:px-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="mt-1 font-heading text-2xl font-bold text-black">
              Zonas de trabajo
            </h1>
          </div>
        ...
        {zonesLoading ? (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {Array.from({ length: 6 }).map((_, index) => (
              <div
                key={index}
                className="h-56 animate-pulse rounded-lg bg-tv-surface"
              />
            ))}
          </div>
        ) : (
