...
COMMENT ON TABLE  empleados                IS 'Registro maestro de los 8 trabajadores. Ref §3.1';
...
COMMENT ON TABLE zonas IS '5 zonas con pantalla TV informativa y tablet interactiva (R3). Ref §3.1';

-- Datos maestros por zona
INSERT INTO zonas (nombre, codigo, tiene_pantalla_tv, tiene_tablet) VALUES
    ('Nave',  'sala_ordeno',    TRUE,  TRUE),
    ('Becerrero',       'becerrero',      TRUE,  TRUE),
    ('Enfermería',      'enfermeria',     TRUE,  TRUE),
    ('Oficina',         'oficina',        TRUE,  TRUE),
...
INSERT INTO maquinaria (nombre, tipo, marca, modelo) VALUES
    ...
    ('Carro TMR','carro_mezclador','X','X'),
    ('Amamantadora','amamantadora','x','x');
...
-- Mapeo real de las pizarras
INSERT INTO tareas_recurrentes (catalogo_id, frecuencia_expr, descripcion_frecuencia, notas)
SELECT id, '0 22 * * 1,4', 'Lunes y jueves a las 22:00', 'Lavado robots 1-2 (lun/xov)'
FROM tareas_catalogo WHERE codigo = 'lavado_robot';

INSERT INTO tareas_recurrentes (catalogo_id, frecuencia_expr, descripcion_frecuencia, notas)
SELECT id, '0 22 * * 2,5', 'Martes y viernes a las 22:00', 'Lavado robots 3-4 (mar/ven)'
FROM tareas_catalogo WHERE codigo = 'lavado_robot';
...
-- Creación de las tareas recurrentes 
...
CREATE INDEX idx_boxes_alertas ON boxes_recria USING GIN(alertas);



-- Dominio 3: Series temporales 
-- lecturas_robot_ordeno
...
SELECT create_hypertable('lecturas_robot_ordeno', 'ts',
...
SELECT create_hypertable('lecturas_meteorologia', 'ts',
    chunk_time_interval => INTERVAL '1 month');


-- Carro Mezcaldor (Pendiente de saber que parámetros se pueden medir)
...
SELECT create_hypertable('lecturas_carro_mezclador', 'ts',
    chunk_time_interval => INTERVAL '1 month');
...
-- Umbrales iniciales (Pendientes de revisión)
...
-- Aplicamos estos a las 5 tablas críticas
...
-- Vistas en las pantallas (Pendiente revisar las vistas necesarias y ajustar segúin necesidades)

-- Vista: Produccion diaria por animal con datos maestros
...
-- Vista: Vacas con período de retirada activo
...
-- Vista: Tratamientos activos con checklist pendiente
